...
object TagsContext {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //docs:字典文件  app_dict.txt      day:自己传入，比如：20190922
    ...
    // 读取数据文件   源数据文件  文件中的每一行就是一个ROW对象类型
    ...
    df.map(row=>{
      ...
      case (userId,userTags) =>{
        ...
        put.addImmutable(Bytes.toBytes("tags"),Bytes.toBytes(day),Bytes.toBytes(userTags.mkString(",")))//设置列和值
